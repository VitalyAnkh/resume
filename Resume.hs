...
basicInfo = paragraph
  [ pure "\\basicInfo{"
  , pure $ "\\email{vitalyankh@gmail.com}" ++ period
  -- , cn $ "\\phone{(+86) 180-8192-5082}" ++ period
  -- , en $ "\\phone{(1) 717-728-6526}" ++ period
  , pure $ "\\github[VitalyAnkh]{https://github.com/VitalyAnkh}" ++ period
  , pure $ "\\homepage[vitalyr.com]{https://vitalyr.com}" ++ period
  -- , pure "\\linkedin[vitalyr]{https://www.linkedin.com/in/vitalyr}"
  , pure "}"
  ] where period = "\\textperiodcentered\\"

education :: Resume
education = section "教育经历" "Education"
  [ datedSection (date "2016" "09" ~~ present) $ paragraph
    [ en "\\textbf{Beijing Jiaotong University }, BJTU, China"
    , cn "\\textbf{ 北京交通大学 }, 中国"
    ]
  , en $ "Major: Computer Science (Undergraduate), Anticipated Graduation Date: 06/2022, "
  , cn $ "专业：信息与计算科学（本科），预计毕业日期： 2021 年 6 月， " 
  ] 
...
--     , cn $ "\\item 设计与实现更多实用语法糖、IDE 插件功能，如 section 语法、子表达式访问器等。"
--     , en $ "\\item Designed and implemented syntax sugars and IDE features, such as the section syntax, subexpr visitor, etc."

--     , cn $ "\\item 开发了一个可扩展的 REPL 框架，并提供命令行实现（可基于上下文补全）、IDE 实现（可与 IDE 中打开的项目进行交互）。"
--     , en $ "\\item Created an extensible REPL framework, provided implementations in CLI (with context-based completion) and in IDE (interacts with the modules loaded in the opened project)."
--     ]
--   ]
...
skills = section "技能" "Skills" . pure $ itemize
  ...
  , itemTeX "DevOps" "DevOps"
  , cn "熟练使用Ansible ，Docker 等 DevOps 工具，"
  , cn $ "维护使用 Ansible 进行配置管理的 Arch Linux 桌面" ++ arch_ansible
      
  , itemTeX "范畴论" "Category Theory"
  , cn "掌握范畴论的概念，原理和方法，"
  , cn "习惯使用范畴的观点理解并重构数学和计算机科学"

  , itemTeX "数据科学" "Data Science"
  , cn "掌握Python, R 语言以及相关的数据分析技能。"
 ...
 , cn "掌握类型论，正在学习同伦类型论；"
  ...
  , cn "熟悉 Idris, Agda 和一些 Coq。"
  , en "familiar with Idris, Agda ,"
  , en "F$\\star$, Arend and some Coq."

  , let last   = "Agda"
        others = "Idris2, owo-lang"
    in paragraph
  ...
  , noSimple $ paragraph
    [ pure "\\item \\textbf{JetBrains MPS, Racket}:"
  ...
  where
      somehow = "C++ Java Kotlin Idris"
      very    = "Rust C Haskell Agda ELisp Racket Python"
      arch_ansible = "\\href{https://github.com/VitalyAnkh/config} {Arch Ansible}"
...
resume = paragraph
  ...
  --   in itemize
    [
        cn "Emacs 熟练用户，正在尝试给Emacs 添加用Rust 写的动态模块"
    --, cn "熟练使用 Mathematica "
